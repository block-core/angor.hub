...
export class IndexerService {
    ...
    getIndexers(network: 'mainnet' | 'testnet'): string[] {
        const storageKey =
            network === 'mainnet'
                ? this.mainnetLocalStorageKey
                : this.testnetLocalStorageKey;
        return JSON.parse(localStorage.getItem(storageKey) || '[]');
    }

    private saveIndexers(
        indexers: string[],
    ...
    setPrimaryIndexer(indexer: string, network: 'mainnet' | 'testnet'): void {
        if (this.getIndexers(network).includes(indexer)) {
            const primaryKey =
                network === 'mainnet'
                    ? this.mainnetPrimaryIndexerKey
                    : this.testnetPrimaryIndexerKey;
    ...
    getNetwork(): 'mainnet' | 'testnet' {
        return (
            (localStorage.getItem(this.networkStorageKey) as
                | 'mainnet'
                | 'testnet') || 'testnet'
        );
    }
